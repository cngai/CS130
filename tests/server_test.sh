...
grep "^$(date +'\[%Y-%m-%d')" $PATH_TO_STATIC/SYSLOG_0.log|tail -8|grep "Handle ping"
if [[ $? -eq 0 ]]; then
...
echo "Covid HANDLER TEST ............."
echo "curl localhost:$PORT1/covid"
curl -s localhost:$PORT1/covid > received_covid_page.html
cat covid_index.html > expected_covid_page.html

if [[ $? -ne 0 ]]; then
    echo -e "\e[31mSTATUS: COVID CONNECTION ERROR${NOCOLOR}"; 
    kill -9 $pid_server1
    exit 1;
fi

cmp expected_covid_page.html received_covid_page.html

if [[ $? -eq 0 ]]; then
    echo -e "${GREEN}COVID HANDLER TEST1 PASS${NOCOLOR}"; 
else 
    echo -e "${RED}COVID HANDLER TEST1 FAILED${NOCOLOR}"; 
    kill -9 $pid_server1
    exit 1;
fi
curl -s $IP/covid > GCP_received.html


cmp expected_covid_page.html GCP_received.html

if [[ $? -eq 0 ]]; then
    echo -e "${GREEN}COVID HANDLER TEST2 PASS${NOCOLOR}"; 
else 
    echo -e "${RED}COVID HANDLER TEST2 FAILED${NOCOLOR}"; 
    kill -9 $pid_server1
    exit 1;
fi

rm received_covid_page.html
rm expected_covid_page.html
rm GCP_received.html
